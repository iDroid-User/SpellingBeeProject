...
public class SpellingBee {
    ...
    private void solveAction() {
        String puzzle = sbg.getBeehiveLetters().trim(); // Fetch data at the top level, then pass it down to helpers!
...
//        s = s.toUpperCase(); // Uniform case
    ...
    private ArrayList<String> validateWords(String p) {
        ...
        for (String word : words) {
            if (isWordLegal(word, p)) legalWords.add(word);
        }
        return legalWords;
    }

    // Decompose the solveAction problem: display scores following the words that they credit
    private int scoreWord(String w, String puzz) {
        int score = 1;
        if (w.length() > 4) {
            score *= w.length(); // Words 4 letters or longer score a point per letter
            // A 7-letter word is likely a pangram
            if (w.length() >= 7) {
                for (int i = 0; i < puzz.length(); i++) {
                    // Make character comparison case-insensitive
                    if (w.indexOf(Character.toLowerCase(puzz.charAt(i))) == -1) return score; // Not a pangram
                }
